...
## Deploying Jenkins to Azure Kubernetes Service
...
The template will provision the following resources in Azure:
...
1. Go to [this link](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FAzure%2Fazure-quickstart-templates%2Fmaster%2F301-jenkins-aks-zero-downtime-deployment%2Fazuredeploy.jsonClick)  to get to the form in the portal.

**Note and record the admin password that you provide in the form**.  
 
While the deployment is running and deploying resources we'll discuss what's happening under the hood in a presentation.
...
1. Visit http://localhost:8080, login with the admin password you provided in the form.
...
1. You can access the AKS cluster by doing the following from your local machine.
   ...
   To verify the connection to your cluster, use the `kubectl get` command.  The result should look something like this:
...
1. If you run the build more than once, it will cycle through BLUE and GREEN deployments. i.e., if the current
   ...
   public endpoint to route traffic to the **Blue** environment if all is good with
